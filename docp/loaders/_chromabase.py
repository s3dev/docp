...
class _LoadChromaBase:
    ...
    _PARSERS = {'.pdf': ParsePDF}

    # def __init__(self, path: str, db: str | Chroma, collection: str=None):
    ...
    def _create_documents(self) -> bool:
        ...
        """
        self._docs = [Document(page_content=text['content'],
                               metadata={'source': self._p.doc.basename,
                                         'pageno': text['pageno']})
                      for text in self._p.doc.text if text['content']]
        if not self._docs:
    ...
    def _parse_text(self) -> bool:
        ...
        self._p.extract_text(remove_header=True, remove_footer=True, convert_to_ascii=True)
        if not self._p.doc.text:
    ...
    def _set_parser(self) -> bool:
        ...
        :Rationale:
            ...
            extension ``.pdf`` will set the :class:`parsers.pdf.ParsePDF`
            class.
    ...
    def _test_load(self, nrecs_b: int, nrecs_a: int) -> bool:
        ...
        """
        if nrecs_a == nrecs_b:
            ui.print_warning(f'{_PRE_WARN} No new documents added.')
