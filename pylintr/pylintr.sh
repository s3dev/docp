...
for f in $( /usr/bin/find ../ -name "*.py" | grep -v "docs" ); do
    ...
    [ $dname = ".._" ] && dname=""
    #
    # HACK: 1) Updated to lint modules with a leading underscore.
    #       2) Excluding __init__ and _version
    #
    #   Consider adding this to a future update.
    #
    #if [[ ${bname} =~ ^[a-z][a-z0-9_]+\.py ]]; then
    if [[ ${bname} =~ ^[_a-z][a-z0-9_]+\.py ]] \
       && [ ${bname} != "_version.py" ] \
       && [ ${bname} != "__init__.py" ]; then
        echo Processing: ${f}
